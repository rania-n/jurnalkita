<x-layouts.app title="Jurnal Pengganti">
    ...
    @else
        <form method="POST" action="{{ route('sekretaris.jurnal.pengganti.store') }}" class="flex flex-col gap-4">
            @csrf

            <x-ui.select label="Mata Pelajaran (jadwal)" name="jadwal_id">
                <option value="" disabled selected hidden>Pilih jadwal</option>
                @foreach ($jadwals as $j)
                    <option value="{{ $j->id }}" data-mulai="{{ $j->jam_ke_mulai }}" data-selesai="{{ $j->jam_ke_selesai }}">
            ...
            <x-ui.textarea label="Materi / Tugas yang diberikan" name="materi" :rows="3" placeholder="Contoh: mengerjakan LKS halaman 12–15.">{{ old('materi') }}</x-ui.textarea>
            <x-ui.textarea label="Tugas Tambahan (opsional)" name="tugas_tambahan" :rows="2">{{ old('tugas_tambahan') }}</x-ui.textarea>
