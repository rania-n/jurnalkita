    ...
    @if ($admin)
        <x-admin.page title="Dispensasi Siswa" subtitle="Persetujuan izin keluar / tidak mengikuti pelajaran">
            <x-slot:action>
                @if ($bolehEkspor)
                    <a href="{{ $urlEkspor }}"
                       class="press inline-flex h-7 shrink-0 items-center justify-center gap-2.5 rounded-md bg-surface-alt px-2.5 text-xs font-semibold text-ink hover:bg-[#cbd5e1]">
                ...
                @if ($bolehAjukan)
                    <a href="{{ route('dispensasi.create') }}"
                       class="press inline-flex h-7 shrink-0 items-center justify-center gap-1 rounded-md bg-navy px-2.5 text-xs font-semibold text-card hover:bg-navy-hover">
