@props(['action', 'hideButtons' => false])
...
<form method="GET" action="{{ $action }}" class="mb-4 flex flex-wrap items-end gap-3">
    ...
    @if (!$hideButtons)
        <button type="submit" class="press flex h-10 shrink-0 items-center gap-1.5 rounded-lg border border-surface-alt bg-card px-3 text-sm font-semibold text-ink hover:bg-surface">
