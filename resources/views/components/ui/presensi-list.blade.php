...
<div class="flex flex-col divide-y divide-surface-alt overflow-hidden rounded-xl border border-surface-alt bg-card">
    @foreach ($absensis as $a)
        <div class="flex items-center gap-3 px-4 py-2.5">
            ...
            @if ($linkSiswa)
                <a href="{{ route('guru.siswa.show', $a->siswa) }}" class="min-w-0 flex-1 truncate text-sm font-medium text-ink hover:text-navy hover:underline">{{ $a->siswa->nama }}</a>
            @else
                <span class="min-w-0 flex-1 truncate text-sm font-medium text-ink">{{ $a->siswa->nama }}</span>
            @endif
