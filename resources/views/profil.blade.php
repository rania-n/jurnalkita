...
<x-layouts.app title="Profil">
    ...
    <div class="flex flex-col items-center gap-3 py-2">
        <span class="flex h-20 w-20 items-center justify-center rounded-full bg-navy text-2xl font-bold text-card">
            {{ $inisial }}
        </span>
    ...
    <div class="flex flex-col gap-3">
        ...
        @if ($guru)
            ...
            <x-ui.field-static label="Mata Pelajaran Utama" icon="menu_book">
                {{ $guru->mapelUtama->nama ?? '—' }}
            </x-ui.field-static>
            ...
            @if ($guru->kelasWali->isNotEmpty())
                <x-ui.field-static label="Wali Kelas" icon="groups">{{ $guru->kelasWali->pluck('nama')->join(', ') }}</x-ui.field-static>
            @endif
        @elseif ($siswa)
