...
class PengaturanJurnalTest extends TestCase
{
    ...
    public function test_halaman_pengaturan_render(): void
    {
        $this->actingAs($this->admin)->get('/admin/pengaturan-jurnal')
            ->assertOk()->assertSee('Disiplin')->assertSee('Bebas isi hari ini');
    }

    public function test_admin_bisa_ubah_mode(): void
    {
        $this->actingAs($this->admin)->post('/admin/pengaturan-jurnal', ['mode' => 'bebas_kemarin'])
            ->assertRedirect();

        $this->assertSame('bebas_kemarin', PengaturanJurnal::mode());
    }
